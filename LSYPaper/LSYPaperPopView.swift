...
class LSYPaperPopView: UIView {
    ...
    @IBOutlet weak var triangleImageTopConstraint: NSLayoutConstraint!
     var panPop:UIPanGestureRecognizer = UIPanGestureRecognizer()

    override func layoutSubviews() {
        frame = targetFrame
    }
    
    class func showPopViewWith(_ frame:CGRect,viewMode:LSYPaperPopViewMode,inView:UIView,frontView:UIView,revokeOption:@escaping (() -> Void)) {
        var name:String = ""
    ...
    class func hidePopView(_ fromView:UIView) {
     ...
     class func getPopViewFrom(_ fromView:UIView) -> LSYPaperPopView {
        let subViewsEnum = fromView.subviews.reversed()
        var popView = LSYPaperPopView()
        for subView in subViewsEnum {
            if subView.isKind(of: self) {
     ...
     class func getBackgroundFrom(_ fromView:UIView) -> UIView {
        let subViewsEnum = fromView.subviews.reversed()
